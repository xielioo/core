...
class OC_Files {
	...
	public static function get($dir, $files, $params = null) {
		...
		try {
			...
			OC::$server->getLogger()->logException($ex);
			$l = \OC::$server->getL10N('core');
			\OC_Template::printErrorPage($l->t('File, folder or one of the files inside the folder cannot be downloaded'), $ex->getMessage(), 403);
	...
	private static function getSingleFile($view, $dir, $name, $params) {
		...
		if (\OC\Files\Filesystem::isReadable($filename) && !$event->hasArgument('errorMessage')) {
			...
			if ($event->hasArgument('errorMessage')){
				$msg = $msg . ': ' . $event->getArgument('errorMessage');
			}
